# 《팔대신병록》 설정·설계 허브

상태: `active-index`
상위 정책: [`book_writing.md`](book_writing.md)
진행 현황: [`operations/PM-PROGRESS-DASHBOARD.md`](operations/PM-PROGRESS-DASHBOARD.md)

---

# 현재 상태

- Gate 0 — 프로젝트 경계: `resolved-provisional`
- Gate 1 — 역사·세계 골격: `resolved-provisional`
- Gate 2 — 핵심 시스템: `resolved-provisional`
- Gate 3 — 원사건·인물·핵심 갈등: `resolved-provisional`
- Gate 4 — 결말·전체 플롯·복선: `resolved-provisional`
- Gate 5 — 집필 준비·최종 감사: `resolved-provisional`

설정 설계: **완료 / Human Edit 대기**

원고 구조:

- 전체 시놉시스 초안 완료
- 6부 원고 개요 초안 완료
- 60장 작업 개요 초안 완료
- 다음 단계: 장면 카드

---

# 최우선 설정 기준

- [통합 설정집 v1.0](design/SETTING-BIBLE-v1.0.md)
- [Gate 5 역사 지명 교정 부록](operations/GATE-5-HISTORY-ADDENDUM.md)
- [Gate 5 집필 준비 감사](audit/GATE-5-READINESS-AUDIT.md)
- [Gate 5 결정](operations/GATE-5-DECISION.md)

지명에 한해 역사 교정 부록이 통합 설정집의 레거시 작업명을 우선한다.

- 해문도 → 쇄조도
- 해문합증 → 쇄조합증

과거 후보 문서와 충돌할 경우 최신 Gate 결정·교정 부록·통합 설정집을 우선한다.

---

# 원고 설계

- [전체 시놉시스 v0.1](manuscript/SYNOPSIS-v0.1.md)
- [6부 원고 개요 v0.1](manuscript/ARC-OUTLINE-v0.1.md)
- [60장 작업 개요 v0.1](manuscript/CHAPTER-OUTLINE-v0.1.md)
- [선택 결말](design/SELECTED-ENDING-v0.1.md)
- [전체 대서사](design/MASTER-PLOT-v0.1.md)
- [부별 구조](design/ARC-BREAKDOWN-v0.1.md)
- [주요 전환점](design/TURNING-POINTS-v0.1.md)

60장은 작업 구조이며 장면 리듬 감사에서 54~66장 범위로 조정할 수 있다.

---

# 집필 규칙

- [장면 설계 규칙](writing/SCENE-DESIGN-RULES.md)
- [전투 설계 지침](writing/COMBAT-DESIGN-GUIDE.md)
- [시점·문체·정보 공개 규칙](writing/WRITING-AND-INFORMATION-RULES.md)

---

# 핵심 설정

## 역사·세계

- [역사·지리 기반](design/HISTORICAL-BASELINE-v0.1.md)
- [세계 권력 작동 원리](design/WORLD-POWER-SYSTEM-v0.1.md)
- [강호 제도](design/JIANGHU-INSTITUTIONAL-SYSTEM-v0.1.md)
- [권역 구조](design/REGIONAL-STRUCTURE-v0.1.md)
- [경제·물류](design/ECONOMY-AND-LOGISTICS-v0.1.md)

## 무공·의술·기환

- [무공 상위 체계](design/MARTIAL-SYSTEM-FOUNDATION-v0.1.md)
- [부상·의술·독](design/INJURY-MEDICINE-POISON-SYSTEM-v0.1.md)
- [기환 상한](design/SUPERNATURAL-LIMITS-v0.1.md)
- [기환 하위 현상](design/SUPERNATURAL-SUBSYSTEMS-v0.1.md)

## 병기·증거·팔대신병

- [병기·유물 분류](design/ARTIFACT-CLASSIFICATION-SYSTEM-v0.1.md)
- [증거·감정·위조](design/EVIDENCE-AUTHENTICATION-SYSTEM-v0.1.md)
- [팔대신병 정의](design/EIGHT-DIVINE-ARMS-DEFINITION-v0.1.md)
- [신병 잔흔](design/ARTIFACT-MEMORY-SYSTEM-v0.1.md)
- [신병 권리·합증](design/ARTIFACT-RIGHTS-CONVERGENCE-SYSTEM-v0.1.md)
- [현재 팔대신병](design/CURRENT-EIGHT-DIVINE-ARMS-v0.1.md)
- [현재 보관 지도](design/CURRENT-ARTIFACT-CUSTODY-MAP-v0.1.md)

## 원사건·인물·세력

- [쇄조도 원사건 — 레거시 파일명](design/ORIGINAL-EVENT-v0.1.md)
- [최초 여덟](design/ORIGINAL-EIGHT-PARTIES-v0.1.md)
- [진실 층위](design/TRUTH-LAYERS-v0.1.md)
- [인물 바이블](design/CHARACTER-BIBLE-v0.1.md)
- [관계도](design/RELATIONSHIP-MAP-v0.1.md)
- [핵심 세력 등록부](design/CORE-FACTION-REGISTRY-v0.1.md)
- [사문경 계획](design/ANTAGONIST-PLAN-v0.1.md)

---

# 핵심 장부

- [미해결 질문](audit/OPEN-QUESTIONS.md)
- [설정 백로그](audit/SETTING-BACKLOG.md)
- [전체 플롯 연표](audit/PLOT-TIMELINE.md)
- [이동 시간](audit/TRAVEL-TIME-LEDGER.md)
- [연령·생애](audit/AGE-BIOGRAPHY-LEDGER.md)
- [부상·회복](audit/INJURY-CONTINUITY-LEDGER.md)
- [관계·신뢰](audit/RELATIONSHIP-PROGRESSION-LEDGER.md)
- [인물 지식](audit/CHARACTER-KNOWLEDGE-LEDGER.md)
- [단서·복선](audit/CLUE-LEDGER.md)
- [가짜 가설](audit/FALSE-HYPOTHESIS-LEDGER.md)
- [신병 플롯·이동](audit/ARTIFACT-PLOT-LEDGER.md)
- [현재 범죄 책임](audit/CURRENT-CRIME-RESPONSIBILITY-LEDGER.md)
- [인물 결말](audit/CHARACTER-ENDING-LEDGER.md)
- [역사·용어 감사](audit/HISTORICAL-TERMINOLOGY-AUDIT.md)
- [용어 변경 기록](audit/TERMINOLOGY-CHANGE-LOG.md)

---

# 역사·용어 교정

새 원고 설계에서 우선한다.

- 진원표국 → 진원행
- 검표수 → 검화수
- 표사 → 호송인·압운인
- 표두 → 호송두·행관
- 남방 회수별조 → 남방 회수차견·금의위 차견 일행
- 해문도 → 쇄조도
- 해문합증 → 쇄조합증

---

# 운영 결정

- [Gate 0 결정](operations/GATE-0-DECISION.md)
- [Gate 1 결정](operations/GATE-1-DECISION.md)
- [Gate 2 결정](operations/GATE-2-DECISION.md)
- [Gate 3 결정](operations/GATE-3-DECISION.md)
- [Gate 4 결정](operations/GATE-4-DECISION.md)
- [Gate 5 결정](operations/GATE-5-DECISION.md)
- [Gate 5 역사 교정 부록](operations/GATE-5-HISTORY-ADDENDUM.md)

---

# 후속 상세화 원칙

지금부터 새 상위 설정을 늘리지 않는다.

장면 카드에 실제로 필요한:

- 지역 인물
- 사건 피해자
- 세부 지명·경로
- 전투 장소
- 문서 형식
- 개별 초식·별호

만 해당 장의 기능에 맞춰 추가한다.

정본 승격과 장면 카드 대량 작성은 사용자 Human Edit 뒤 진행한다.
